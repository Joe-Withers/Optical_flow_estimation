...
def load_my_synthetic_flows(im_path, im_width=128, im_height=128, n_data=100):
    ...
    for i in range(0,n_data):
        #messed up when generating the flow for this dataset do the negative of the flow from image 2 to image 1 is what we want as the flow from image 1 to image 2
        f_flow = im_path+'flow/'+str(i)+'-I2-I1.npy'
        flows[i,:,:,:] = -np.load(f_flow)
    return flows

def load_my_synthetic_flows_filenames(im_path, n_data=100):
    f_flow = [im_path+'flow/'+str(i)+'-I2-I1.npy' for i in range(0,n_data)]
    return f_flow
